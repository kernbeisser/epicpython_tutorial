...
def some_func():
    ...
    set1 = set(list1)
    #creating a set from a list; removing duplicate elements;
    # returns {11, 12, 13, 14, 15}

    set2 = set(string1)
    #creating a set from a string; removing duplicate characters;
    # returns {'b', 'a', 'g', 'f', 'c', 'd', 'e'};
    # remeber that sets are UNORDERED collections of elements
    ...
    #Frozensets - immutable sets. The elements of a frozenset
    ...
    set1.union(set2)
    #unifying two sets; the result is also a set, so there
    # are no duplicate elements; not to be confused with concatenation

    set1.pop()
    #removes a random element from the set; set elements cannot be removed
    #by index because sets are UNORDERED collections of elements, so there
    #are no indexes to use
