...
def some_func():
    ...
    myfile = open("C:\\Users\\john\\routers.txt", "r")
    # "r" is the file access mode for reading and it is the default mode when opening a file
    ...
    # opens/creates a new file for writing; the "w" method also creates the file for writing
    # if the file doesn’t exist and overrides the file if the file already exists;
    # remember to close the file after writing to it to save the changes!
    ...
    newfile = open("newfile.txt", "w+") # opens a file for both writing and reading
    # at the same time

    newfile = open("newfile.txt", "x") # opens for exclusive creation, failing if the file
    # already exists
    ...
    # using the with-as solution, the files gets closed automatically,
    # without needing the close() method
